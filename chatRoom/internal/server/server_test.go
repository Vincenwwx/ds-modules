...
func setupTest(t *testing.T, fn func(*Config)) (
	...
	*/
	...
	// set up the telemetry exporter to write to two files
	...
	return rootClient, nobodyClient, cfg, func() {
		...
		if telemetryExporter != nil {
			// sleep 1.5s to provide enough time for flushing the data to disk
...
func testProduceConsume(t *testing.T, client, _ api.LogClient, config *Config) {
	...
	consumue, err := client.Consume(ctx, &api.ConsumeRequest{
		Offset: produce.Offset,
	})
	require.NoError(t, err)
	require.Equal(t, want.Value, consumue.Record.Value)
	require.Equal(t, want.Offset, consumue.Record.Offset)
}
