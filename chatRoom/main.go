...
func main() {
	...
	cmd := &cobra.Command{
		Use:     "chatRoom",
		PreRunE: cli.setupConfig, // hook functions to run before RunEa
		RunE:    cli.run,
	}
...
func (c *cli) setupConfig(cmd *cobra.Command, args []string) error {
	...
	if err = viper.ReadInConfig(); err != nil {
		// ok even if config file doesn't exist
	...
	if c.cfg.PeerTLSConfig.CertFile != "" && c.cfg.PeerTLSConfig.KeyFile != "" {
		c.cfg.Config.PeerTLCConfig, err = config.SetupTLSConfig(
			c.cfg.PeerTLSConfig,
		)
		if err != nil {
			return err
...
func (c *cli) run(cmd *cobra.Command, args []string) error {
	...
	// shutdown the agent when os terminates the program
